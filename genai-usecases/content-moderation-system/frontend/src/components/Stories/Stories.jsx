...
export default function Stories() {
  ...
  const handleSubmitStory = async () => {
    ...
    let finalContent = storyContent.trim();
    if (storyImageUrl.trim()) {
      finalContent += `\n\n![Image](${storyImageUrl.trim()})`;
    }
    if (storyVideoUrl.trim()) {
      finalContent += `\n\n[Video](${storyVideoUrl.trim()})`;
    }
    if (attachedImage) {
    ...
    try {
      const result = await storiesAPI.submitStory({
        ...
        image_urls: storyImageUrl.trim() ? [storyImageUrl.trim()] : [],
        video_urls: storyVideoUrl.trim() ? [storyVideoUrl.trim()] : [],
