...
export default function ContentReview() {
  ...
  // Determine if content needs action (always allow moderation actions for review page)
    ...
    <Container maxWidth="lg">
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={8}>
          <Card>
            <CardContent>
              ...
              {/* AI Moderation Action (Always available for Admin) */}
              <Box sx={{ mt: 3, mb: 2 }}>
                <Button
                  variant="outlined"
                  size="large"
        ...
        <Grid item xs={12} md={4}>
          ...
          {/* Detailed Multi-Agent Execution Breakdown & Constraints */}
          <Card sx={{ mt: 2 }}>
            <CardContent>
              <Typography variant="h6" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                🤖 Multi-Agent Execution Breakdown & Constraints
              </Typography>
              <Typography variant="caption" color="text.secondary" paragraph>
                Review individual agent reasoning, constraints/thresholds, and final consensus.
              </Typography>
              
              <Divider sx={{ mb: 2 }} />

              {content.agent_decisions_list && content.agent_decisions_list.length > 0 ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {content.agent_decisions_list.map((agentItem, idx) => (
                    <Paper key={idx} variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                        <Typography variant="subtitle2" fontWeight="bold">
                          {agentItem.agent_name}
                        </Typography>
                        <Chip
                          label={`${agentItem.action.toUpperCase()} (${(agentItem.confidence * 100).toFixed(0)}%)`}
                          color={
                            agentItem.action.toLowerCase().includes('approve') ? 'success' :
                            agentItem.action.toLowerCase().includes('warn') || agentItem.action.toLowerCase().includes('flag') ? 'warning' : 'error'
                          }
                          size="small"
                        />
                      </Box>
                      ...
                      <Box sx={{ bgcolor: 'action.hover', p: 1, borderRadius: 1, mb: 1.5 }}>
                        <Typography variant="caption" color="text.secondary" component="div">
                          🎯 <strong>Target Constraint:</strong> {agentItem.constraint}
                        </Typography>
                      </Box>
                      ...
                      {agentItem.flags && agentItem.flags.length > 0 && (
                        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mt: 1 }}>
                          {agentItem.flags.map((flag, fIdx) => (
                            <Chip key={fIdx} label={`🚩 ${flag}`} size="small" variant="outlined" color="warning" />
                          ))}
                        </Box>
                      )}
                    </Paper>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {Object.entries(content.agent_decisions).map(([agentName, decision], idx) => (
                    <Paper key={idx} variant="outlined" sx={{ p: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                        ...
                        <Chip
                          label={`${(decision.action || 'processed').toUpperCase()} (${((decision.confidence || 0.85) * 100).toFixed(0)}%)`}
                          color={
                            (decision.action || '').toLowerCase().includes('approve') ? 'success' :
                            (decision.action || '').toLowerCase().includes('warn') ? 'warning' : 'error'
                          }
                          size="small"
                        />
                      </Box>
                      <Typography variant="caption" color="text.secondary">
                        🎯 Target Constraint: Standard Moderation Thresholds (Toxicity ≤ 40%, Confidence ≥ 70%)
                      </Typography>
                    </Paper>
                  ))}
                </Box>
                ...
                <Alert severity="info">
                  Default moderation criteria applied (Toxicity Threshold: ≤ 40%, Consensus Threshold: ≥ 67%). Run AI Moderation to view detailed step-by-step agent traces.
                </Alert>
              )}
            </CardContent>
          </Card>
