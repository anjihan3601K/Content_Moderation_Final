...
from src.utils.tools import generate_content_id, generate_user_id, analyze_uploaded_image
...
# Global instances
db: FirebaseDatabaseManager = None
sqlite_mod_db: ModerationDatabase = None
...
async def lifespan(app: FastAPI):
    ...
    global db, sqlite_mod_db, auth_db, workflow, ml_status
    ...
    logger.info("\nInitializing databases (Firebase Firestore + SQLite backup)...")
    firebase_project_id = os.getenv("FIREBASE_PROJECT_ID", "finalyearproject-ae788")
    db = FirebaseDatabaseManager(project_id=firebase_project_id)
    sqlite_mod_db = db.sqlite_mod
...
class ContentSubmission(BaseModel):
    ...
    content_text: Optional[str] = Field("", description="Content text to moderate")
    content_type: str = Field("text", description="Type of content (text, image, video, audio, comment, post)")
    ...
    # Optional media URLs for multimodal content
    image_urls: List[str] = Field(default_factory=list, description="List of image URLs to inspect")
    video_urls: List[str] = Field(default_factory=list, description="List of video URLs to inspect")
    audio_urls: List[str] = Field(default_factory=list, description="List of audio URLs to inspect")
    media_urls: List[str] = Field(default_factory=list, description="Generic media URLs for any file type")
...
async def register(request: RegisterRequest):
    ...
    try:
        # Create user with default 'user' role in SQLite auth DB
        ...
        # Dual-sync newly registered user to Cloud Firebase
        if db:
            try:
                db.create_or_update_auth_user({
                    "username": request.username,
                    "password": request.password,
                    "full_name": request.full_name,
                    "role": "user",
                ...
                logger.info(f"Successfully synced newly registered user '{request.username}' to Cloud Firebase")
            except Exception as fe:
                logger.error(f"Error syncing user '{request.username}' to Cloud Firebase: {fe}")
...
async def update_user(user_id: int, request: UpdateUserRequest):
    ...
    try:
        ...
        if db and user.get('email'):
            try:
                db.create_or_update_auth_user({
                    "user_id": user_id,
                    "username": user['username'],
                    "full_name": request.full_name or user['full_name'],
                    "role": request.role or user['role'],
                    "email": request.email or user.get('email'),
                    "phone": request.phone or user.get('phone'),
                    "is_active": request.is_active if request.is_active is not None else user.get('is_active', 1),
                    "created_at": user.get('created_at') or datetime.now().isoformat(),
                    "last_login": user.get('last_login')
                })
                logger.info(f"Synced updated role for '{user['username']}' to Firebase custom claims and user docs")
            except Exception as e:
                logger.error(f"Failed to sync updated role to Firebase for user {user_id}: {e}")
...
async def upload_image_for_moderation(
    ...
    """Validate an actual uploaded image and return a pixel-based moderation risk assessment."""
    if not file.filename:
        raise HTTPException(status_code=400, detail="File name is required")

    file_bytes = await file.read()
    analysis = analyze_uploaded_image(file_bytes, file.filename)

    if not analysis.get("valid"):
        raise HTTPException(status_code=400, detail=analysis.get("error", "Invalid image file"))
    ...
    return {
        ...
        "analysis": analysis,
        "moderation_flag": analysis["is_unsafe"],
    }
...
async def submit_content(submission: ContentSubmission, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        media_image_urls = list(submission.image_urls or [])
        media_video_urls = list(submission.video_urls or [])
        media_audio_urls = list(submission.audio_urls or [])
        media_urls = list(submission.media_urls or [])
        ...
        initial_state: ContentState = {
            ...
            # Image/video/audio analysis (for multimodal content)
            "image_urls": media_image_urls,
            "video_urls": media_video_urls,
            "audio_urls": media_audio_urls,
            "image_descriptions": [],
            "detected_objects": [],
            "detected_text_in_media": [],
            "media_risk_score": 0.0,
            "media_risk_level": "none",
            "media_signals": [],
...
@app.post("/api/content/{content_id}/moderate")
async def re_moderate_content(content_id: str):
    """Re-run AI moderation pipeline on content."""
    try:
        ...
        metadata = ContentMetadata(
            content_id=content_id,
            content_type=content.get("content_type", "story"),
            platform=content.get("platform", "stories"),
        ...
        initial_state: ContentState = {
            "content_id": content_id,
            "submission_id": content.get("submission_id", f"SUB-{content_id}"),
            "submission_timestamp": content.get("submission_timestamp", datetime.now().isoformat()),
        ...
        # Update database with results
        new_status = final_state.get("status", "approved")
        db.update_content_status(
            content_id=content_id,
            status=new_status,
            moderation_action=final_state.get("moderation_action"),
        ...
        # Sync story table if applicable
        is_approved = new_status == ContentStatus.APPROVED.value
        ...
        return {
            ...
            "status": new_status,
            "moderation_action": final_state.get("moderation_action"),
            "message": "AI Moderation re-run successfully"
        }
    except Exception as e:
        logger.error(f"Error in re-moderation endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_content(content_id: str):
    ...
    try:
        ...
        # Transform agent_decisions to detailed list with constraints and reasoning
        agent_decisions_dict = {}
        agent_decisions_list = []

        # Define default target constraints per agent
        agent_constraints_map = {
            "Content Analysis Agent": "Toxicity < 50%, Explicit Content = False, Sensitive Topics = 0",
            "Toxicity Detection Agent": "Toxicity Score ≤ 40% (Threshold = 0.40), Hate Speech = False",
            "Policy Violation Agent": "Policy Violations = None, Severity = Low/None",
            "User Reputation Agent": "User Reputation Score ≥ 0.50, Risk Score ≤ 0.50",
            "ReAct Decision Synthesis": "Consensus Agreement ≥ 67% (2/3 Majority), Confidence ≥ 70%"
        }

        for decision in agent_decisions:
            agent_name = decision.get("agent_name", "unknown")
            conf = decision.get("confidence", decision.get("confidence_score", 0.85))
            ...
            item = {
                ...
                "confidence": conf,
                "reasoning": decision.get("reasoning", "No reasoning recorded"),
                "flags": flags,
                "recommendations": recs,
                "extracted_data": extracted,
                "constraint": agent_constraints_map.get(agent_name, "Standard Moderation Thresholds"),
                "requires_human_review": bool(decision.get("requires_human_review", 0)),
                "processing_time": decision.get("processing_time", 0.0)
            }
            
            agent_decisions_dict[agent_name] = item
            agent_decisions_list.append(item)
        ...
        return {
            ...
            "agent_decisions_list": agent_decisions_list,
...
async def manual_review_by_id(content_id: str, review: dict):
    ...
    try:
        ...
        if content_type in ["story", "post", "text"] or platform in ["stories", "forum", "generic"]:
            ...
            if story:
                ...
                if content_text.startswith("[Title:"):
                    ...
                    if title_end > 0:
                        title = content_text[7:title_end].strip()
...
async def get_stories(visible_only: bool = True, limit: int = 50):
    ...
    try:
        ...
        # Fallback: combine approved content submissions if not already in stories table
        if visible_only:
            approved_content = db.get_content_by_status("approved", limit=limit)
            existing_content_ids = {s.get("content_id") for s in stories if s.get("content_id")}
            ...
            for item in approved_content:
                ...
                if cid and cid not in existing_content_ids:
                    ...
                    if content_text.startswith("[Title:"):
                        ...
                        if title_end > 0:
                            title = content_text[7:title_end].strip()
                            body = content_text[title_end+1:].strip()

                    stories.append({
                        "story_id": f"STORY-{cid}",
                        ...
                        "view_count": 0,
                        "comment_count": 0,
                        "created_at": item.get("submission_timestamp", datetime.now().isoformat())
                    })
