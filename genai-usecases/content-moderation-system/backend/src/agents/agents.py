...
class ContentModerationAgents:
    ...
    def content_analysis_agent(self, state: ContentState) -> ContentState:
        ...
        try:
            ...
            # Analyze media URLs for multimodal moderation
            media_results = []
            for media_type, urls in {
                "image": state.get("image_urls", []),
                "video": state.get("video_urls", []),
                "audio": state.get("audio_urls", []),
            }.items():
                if urls:
                    result = __import__("src.utils.tools", fromlist=["score_media_urls"]).score_media_urls(urls, media_type=media_type)
                    media_results.append({"media_type": media_type, "result": result})

            if media_results:
                aggregated_score = max(item["result"]["risk_score"] for item in media_results)
                signals = sorted({signal for item in media_results for signal in item["result"]["signals"]})
                state["media_risk_score"] = aggregated_score
                state["media_risk_level"] = "severe" if aggregated_score >= 0.75 else "high" if aggregated_score >= 0.5 else "medium" if aggregated_score >= 0.3 else "low" if aggregated_score > 0 else "none"
                state["media_signals"] = signals
                extracted_data["media_analysis"] = media_results
                if signals:
                    flags.append("media_risk_detected")
                    recommendations.append(f"Suspicious media patterns detected: {', '.join(signals)}")
